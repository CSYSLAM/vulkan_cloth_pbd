...
public:
	...
	void loadAssets()
	{
		...
		textureCloth.loadFromFile(getAssetPath() + "textures/fireplace_colormap_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue);
	}
	...
	void prepareGraphics()
	{
		...
		shaderStages[0] = loadShader(getShadersPath() + "computeclothPBDonlydistcons/cloth.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getShadersPath() + "computeclothPBDonlydistcons/cloth.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
		...
		shaderStages[0] = loadShader(getShadersPath() + "computeclothPBDonlydistcons/sphere.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getShadersPath() + "computeclothPBDonlydistcons/sphere.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	void prepareCompute()
	{
		...
		computePipelineCreateInfo.stage = loadShader(getShadersPath() + "computeclothPBDonlydistcons/cloth.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
